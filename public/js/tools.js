...
function drawColorPicker(x,y,H,w,h){//РИСОВАНИЕ ПАЛИТРЫ ЦВЕТОВ H - Hue, цветовой тон от 0 - 360, w,h - ширина и высота квадрата одного цвета
	H = H % 360;
	var y0 = y;
	for(var S = 0; S < 100; S++,x++){
		for(var V = 0; V < 100; V++,y++){
				ctx.fillStyle = toRGB(H,S,99 - V);
...
function colorManipulate(color,action){
  color = color.replace(/\s/g,"");
  var aRGB = color.match(/^rgb\((\d{1,3}[%]?),(\d{1,3}[%]?),(\d{1,3}[%]?)\)$/i);
  switch(action){
	case "invert":
		color = "rgb("+(255-aRGB[1])+","+(255-aRGB[2])+","+(255-aRGB[3])+")";
	break;
	case "object":
		color = {r: aRGB[1],g: aRGB[2],b: aRGB[3]};
	break;
	case "toHex":default:
	 if(aRGB)
	  {
